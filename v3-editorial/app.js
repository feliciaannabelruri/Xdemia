...
(function () {
  ...
  if (stripTrack) {
    const words = ['Learn', 'Research', 'Grow'];
    const unit = words.map(w => `<span class="strip-item">${w}<span class="sep">δ</span></span>`).join('');
    stripTrack.innerHTML = unit.repeat(8);
  }
  ...
  document.querySelectorAll('[data-count]').forEach((el) => watch(el, animateCount, -0.15));

  /* ---------- bento cards revealed via .reveal watcher above ---------- */

  /* ---------- storage gauge ---------- */
  const gaugeFg = document.getElementById('gaugeFg');
  if (gaugeFg) {
    const C = 2 * Math.PI * 50;            // r = 50
    gaugeFg.style.strokeDasharray = C;
    gaugeFg.style.strokeDashoffset = C;
    watch(gaugeFg, () => { gaugeFg.style.strokeDashoffset = C * (1 - 0.16); }, 0.3);
  }
  ...
  if (mods.length) {
    watch(document.getElementById('bbGrid'), () => {
      mods[0].classList.add('on'); mods[3].classList.add('on');
      setInterval(() => {
        mods.forEach(m => m.classList.remove('on'));
        const k = 2 + Math.floor(Math.random() * 3);
        [...mods].sort(() => Math.random() - 0.5).slice(0, k).forEach(m => m.classList.add('on'));
      }, 1400);
    }, 0.25);
  }
  ...
  if (svg) {
    ...
    nodes.forEach((n, i) => {
      const p = el('path', {
        ...
        fill: 'none', stroke: 'rgba(28,26,20,.28)', 'stroke-width': '1.4'
      });
      svg.appendChild(p);
      // animated pulse dot
    ...
    // nodes
    nodes.forEach((n) => {
      svg.appendChild(el('rect', { x: n.x - 42, y: n.y - 22, width: 84, height: 44, rx: 2, fill: '#f4eee2', stroke: 'var(--ink)', 'stroke-width': '1.4' }));
      const t = el('text', { x: n.x, y: n.y + 4, 'text-anchor': 'middle', class: 'node-label' }); t.textContent = n.label; svg.appendChild(t);
    });
    // hub
    svg.appendChild(el('circle', { cx: hub.x, cy: hub.y, r: 42, fill: 'rgba(10,124,136,.10)', stroke: 'var(--teal)', 'stroke-width': '1.6' }));
    ...
    const hubT2 = el('text', { x: hub.x, y: hub.y + 13, 'text-anchor': 'middle', class: 'node-label', 'font-size': '8' }); hubT2.textContent = 'BRIDGE'; hubT2.setAttribute('fill', 'rgba(28,26,20,.55)'); svg.appendChild(hubT2);
    // breathing ring
